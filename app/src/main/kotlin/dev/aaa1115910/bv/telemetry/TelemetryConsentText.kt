...
object TelemetryConsentText {
    ...
    val sections = listOf(
        ...
        "崩溃报告开关默认关闭。开启后，NeoBV 会通过 Firebase Crashlytics 发送崩溃信息和少量排障字段，例如应用版本、构建类型、Android 版本、设备品牌/型号、播放器类型、默认画质、默认编码、弹幕是否开启、代理是否开启、隐身模式是否开启，以及经脱敏后的错误分类和 HTTP 状态码。",
        "匿名使用信息开关默认关闭。开启后，NeoBV 会通过 Firebase Analytics 发送低频匿名事件，例如应用启动、每日活跃、粗略页面名称、视频/直播错误分类和网络类型。它用于判断问题影响范围和优先级，不用于内容推荐或广告。",
